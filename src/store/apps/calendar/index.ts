...
export const addEvent = createAsyncThunk('appCalendar/addEvent', async (event: AddEventType, { dispatch }) => {
  ...
  console.log(event)
  await axios.post('https://clownfish-app-tugh2.ondigitalocean.app/maintenances/', {
    url: event.url,
    title: event.title,
    ...
    documents: '',
    descriptionProbleme: '',
    materielId: event.materielId
  })
  await dispatch(fetchEvents(['Maintenance', 'Done', 'Alert']))
...
export const appCalendarSlice = createSlice({
  ...
  initialState: {
    events: [],
    selectedEvent: null,
    selectedCalendars: ['Maintenance', 'Done Maintenance', 'Alert']
  },
  reducers: {
    ...
    handleAllCalendars: (state, action) => {
      ...
      if (value === true) {
        state.selectedCalendars = ['Maintenance', 'Done', 'Alert']
      } else {
