...
import Viewer from 'src/views/components/Viewer'
...
const UserViewLeft = (props : any) => {
  const {setGlobalModelProp} = props
  const [globalModel, setGlobalModel] = useState<any>(null)
  const [specificModel, setSpecificModel] = useState<any>(null)
  const [viewerData, setViewerData] = useState<any>(null)
  const [progress, setProgress] = useState(0)
  useEffect(() => {
    if (globalModel) {
      let modelsLoaded = 0
      globalModel.addEventListener(Autodesk.Viewing.PROGRESS_UPDATE_EVENT, (e: any) => {
        setProgress(e.percent)
      })

      globalModel.addEventListener(Autodesk.Viewing.OBJECT_TREE_CREATED_EVENT, () => {
        modelsLoaded++
        if (modelsLoaded === globalModel.impl.modelQueue().getAllModels().length) {
          setIsLoaded(true)
          setViewerData(globalModel)
        }
      })
    }
  }, [globalModel])
  ...
  useEffect(() => {
    if (globalModel) setIsLoaded(true)
  }, [globalModel])
  // ** States
    ...
    return (
      <Grid container spacing={6}>
        <Grid item xs={12}>
          <Card>
            {/* <Viewer
              setGlobalModel={function (props: Autodesk.Viewing.Viewer3D): void {
                throw new Error('Function not implemented.')
              }}
              setSpecificModel={function (props: Autodesk.Viewing.PropertyResult): void {
                throw new Error('Function not implemented.')
              }}
            /> */}
            {globalModel && <ModelsList viewer={viewerData} />}
            <Viewer
              setGlobalModel={(props: any) => {setGlobalModel(props)
                setGlobalModelProp(props)
              }}
              setSpecificModel={(props: any) => setSpecificModel(props)}
            />
