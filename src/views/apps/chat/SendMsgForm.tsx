...
const SendMsgForm = (props: SendMsgComponentType) => {
  ...
  useEffect(() => {
    ...
    data.map((item: any) => {
      let selectedChat = item.selectedChat
      let message = item.message

      dispatch(sendMsg({ ...selectedChat, message: message }))
    })
  }, [])
  ...
  const handleSendMsg = (e: SyntheticEvent) => {
    ...
    if (store && store.selectedChat && msg.trim().length) {
      ...
      console.log(store.selectedChat, msg)
      // save to local storage in a list of objects
      const data = JSON.parse(localStorage.getItem('chatData') || '[]')
      const value = { selectedChat: store.selectedChat, message: msg }
      data.push(value)
