...
import UploadDocuments from 'src/pages/dashboards/uploaddocuments'
...
const StepperAlternativeLabel = () => {
  ...
  const submitBat = async () => {
    let batiments: string[] = []
    buildingNames.forEach(async (item, index) => {
      const res = await axios.post(url_base + 'batiment', {
        name: item
      })
      batiments.push(res.data.data._id)
    })
  }
  ...
  //         images: ['test1', 'test2'],
  //         models: ['test1', 'test2'],
  //         buildingsName: ['test1', 'test2'],
  //         batiments: ['test1', 'test2']
  //       },
  //       {
  //         headers: {
      ...
      case 2:
        return (
          <Fragment key={step}>
            ...
            <UploadDocuments batimentIdsProps={buildingNames} />
          </Fragment>
        )
      default:
        return 'Unknown Step'
          ...
          <Grid container spacing={5}>
            ...
            <Grid item xs={12} sx={{ display: 'flex', justifyContent: 'space-between' }}>
              ...
              {activeStep === steps.length - 2 ? (
                <Button size='large' variant='contained' onClick={handleNext} disabled={usersData.length === 0}>
                  Suivant
                </Button>
              ) : null}
              {activeStep === steps.length - 1 ? (
                <Button size='large' variant='contained' onClick={handleNext} disabled={selectedModels.length === 0}>
                  Confirmer
                </Button>
              ) : null}
