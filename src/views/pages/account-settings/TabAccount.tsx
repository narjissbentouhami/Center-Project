// ** React Imports
import { useState, ElementType, ChangeEvent, use, useEffect } from 'react'
...
const TabAccount = () => {
  ...
  useEffect(() => {
    fetch('http://localhost:8000/entretiens/get')
      .then(res => res.json())
        ...
        >
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            ...
            {/* <CustomChip
              ...
              skin='light'
              color='primary'
              label={item.technicienID}
              sx={{ textTransform: 'uppercase' }}
            /> */}
          </Box>
          <Box sx={{ my: 2.5, display: 'flex', alignItems: 'center' }}>
            <Typography sx={{ mr: 3, color: 'text.secondary', fontWeight: 600 }}>{item.document}</Typography>
            {item.documents.length > 0 && (
              <Box component='span' sx={{ display: 'flex', cursor: 'pointer', color: 'text.secondary' }}>
                <IconButton size='small' component={Link} href={`${item.documents}`}>
                  <Icon icon='mdi:document' fontSize={20} />
                </IconButton>
              </Box>
            )}
          </Box>
          <Typography sx={{ color: 'text.secondary' }}>Created on {formatTime(item.end)}</Typography>
        </Box>
      ))}
